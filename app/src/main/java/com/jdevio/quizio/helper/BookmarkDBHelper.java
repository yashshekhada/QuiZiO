...
import com.jdevio.quizio.model.Question;
...
public class BookmarkDBHelper extends SQLiteOpenHelper {
    ...
    //database name
    private static String db_name = "quiz_bookmark.db";
    //table name
    ...
    public static String OPTION_E = "option_e";
  ...
  /*  public void insertIntoDB(int que_id, String question, String answer, String solution, String image_url,String option_a,String option_b,String option_c,String option_d) {
    ...
    public void insertIntoDB(int que_id, String question, String answer, String solution, String image_url, String option_a, String option_b, String option_c, String option_d, String option_e) {
        SQLiteDatabase db = this.getWritableDatabase();
        try {
    ...
    public ArrayList<Question> getAllBookmarkedList() {
        ArrayList<Question> bookmarkList = new ArrayList<>();
        ...
        if (c.moveToFirst()) {
            do {
                ...
                book.setTrueAns(c.getString(c.getColumnIndex(ANSWER)));
                book.setId(c.getInt(c.getColumnIndex(QUE_ID)));
                book.setNote(c.getString(c.getColumnIndex(SOLUTION)));
                book.setImage(c.getString(c.getColumnIndex(IMAGE_URL)));
                book.addOption(c.getString(c.getColumnIndex(OPTION_A)).trim());
                book.addOption(c.getString(c.getColumnIndex(OPTION_B)).trim());
                book.addOption(c.getString(c.getColumnIndex(OPTION_C)).trim());
                book.addOption(c.getString(c.getColumnIndex(OPTION_D)).trim());
                if (Session.getBoolean(Session.E_MODE, getApplicationContext()))
                    if (!c.getString(c.getColumnIndex(OPTION_E)).isEmpty())
                        book.addOption(c.getString(c.getColumnIndex(OPTION_E)).trim());
